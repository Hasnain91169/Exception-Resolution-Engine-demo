...
import { createCustomerMessage, createInternalSummary } from '@/lib/exception/templates'
...
export async function POST(req: Request) {
  ...
  // 3) Options
  const options = generateOptions(trigger.cause)

  // 4) Recommendation
  const recommendation = recommendOption({ severityLevel: severity.level, severityScore: severity.score, cause: trigger.cause, delay_hours: trigger.delay_hours, options })

  // Allow override selected option
  const selected_option_id = body.selected_option_id || recommendation.recommended_option_id
  const selected_option = options.find((o) => o.option_id === selected_option_id)

  // 5) Comms
  const recommended_option = options.find((o) => o.option_id === recommendation.recommended_option_id) || options[0]
  const { customer_message, new_eta } = createCustomerMessage({ shipmentId: shipment.shipment_id, cause: trigger.cause, recommended: recommended_option, predicted_eta: shipment.predicted_eta })
  const internal = createInternalSummary({ shipmentId: shipment.shipment_id, goods: shipment.goods_type, severityLevel: severity.level, severityScore: severity.score, options, recommended: recommended_option })

  const mode: 'simulate' | 'execute' = body.mode === 'execute' ? 'execute' : 'simulate'
  const executed = mode === 'execute'

  // tasks for execute mode
  const internal_tasks = executed ? ['Update TMS route', 'Notify carrier', 'Send customer comms'] : ['Update TMS route (planned)', 'Notify carrier (planned)', 'Prepare customer comms (planned)']

  // 6) Audit
  const audit_event = createAuditEvent({ shipment_id: shipment.shipment_id, trigger, severity, options, decision_mode: mode, recommended_option_id: recommendation.recommended_option_id, selected_option_id: selected_option_id, customer_message, internal_summary: internal.internal_summary, executed })
  ...
  const response = {
    exception: {
      ...
      options: options.map((o) => ({ option_id: o.option_id, title: o.title, delta_hours: o.delta_hours, cost_eur: o.cost_eur, risk: o.risk, steps: o.steps })),
      recommendation: {
